...
(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		defaults: {
			...
			tpl: {
				...
				image    : '<img class="fancybox-image" src="{href}" alt="" />{txt}',
				iframe   : '<iframe id="fancybox-frame{rnd}" name="fancybox-frame{rnd}" class="fancybox-iframe" frameborder="0" vspace="0" hspace="0"' + ($.browser.msie ? ' allowtransparency="true"' : '') + '></iframe>',
				error    : '<p class="fancybox-error">The requested content cannot be loaded.<br/>Please try again later.</p>',
				closeBtn : '<a title="Close" class="fancybox-item fancybox-close" href="javascript:;"><span>&nbsp;</span>Back</a>',
		...
		_afterLoad: function () {
			...
			txt = current.txt;
			// overlay background
            //$('.fancybox-overlay').css('background-image', 'url(' + href +')');
            //$('.fancybox-overlay').wrapInner('<div class="fancybox-overlay-bg" />');
		...
		_afterZoomIn: function () {
			...
			if (current.arrows && F.group.length > 1) {
				if (current.loop || current.index > 0) {
					...
					$(current.tpl.prev).insertBefore(F.wrap).bind('click.fb', F.prev);
				}

				if (current.loop || current.index < F.group.length - 1) {
				    $('.fancybox-next').remove();
					$(current.tpl.next).insertBefore(F.wrap).bind('click.fb', F.next);
				}
			}
	...
	F.helpers.overlay = {
		...
		onReady: function (opts, obj) {
			...
			if (!this.overlay) {
				$.extend(this, {
					//overlay : $('<div class="fancybox-overlay"><img src="/" class="fancybox-overlay-bg"></div>').appendTo( obj.parent ),
					overlay : $('<div class="fancybox-overlay"><div class="fancybox-overlay-bg"></div></div>').appendTo( obj.parent ),
