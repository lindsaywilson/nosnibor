...
function corolla_preprocess_html(&$vars) {
  ...
  if ($u == 'slideshow') {
      $vars['theme_hook_suggestions'][] = 'html__slideshow';
  }
  ...
  // Fancybox
  if (stripos($_SERVER['REQUEST_URI'], 'artwork-gallery') !== false) { 
   //drupal_add_js(drupal_get_path('theme', 'corolla') .'/js/fancyBox/source/helpers/jquery.fancybox-buttons.js', 'file');
   drupal_add_js("jQuery(document).ready(function($) {  
     $('.fancybox').fancybox({
			helpers : {
				...
				afterLoad : function() {
					this.title =  'Image ' + (this.index + 1) + ' of ' + this.group.length + (this.title ? ' - ' + this.title : '') + ' &nbsp;&nbsp; / &nbsp;&nbsp; <a href=\"#\" onClick=\"jQuery(\'.artwork-info-pop\').fadeToggle();return false;\" class=\"show-info\">Info  &nbsp; >></a>';
				}
			});

       });
	   ", array('type' => 'inline', 'scope' => 'header', 'weight' => 20));
  } 
  //---------------------------------------------------------------------//
